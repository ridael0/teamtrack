...
const initialEmployeeState: Omit<Item, 'id'> = {
  name: "",
  email: "",
  location: "",
  department: "",
...
export default function   DialogAddEmployee ({children}: PropsWithChildren) {
  const [employee, setEmployee] = useState<Omit<Item, 'id'>>(initialEmployeeState);
  const [isError, setIsError] = useState(false);
  const [isSuccess, setIsSuccess] = useState(false);
  const [isLoading, setIsLoading] = useState(false);

  const btnAddRef = useRef<null | HTMLButtonElement>(null);
  ...
  const handleAdd = async () => {
    try {
      btnAddRef.current?.setAttribute("disabled", "");
      setIsLoading(true);
      setIsError(false);
      setIsSuccess(false);
      //const res =
      ...
      setIsLoading(false);
      btnAddRef.current?.removeAttribute("disabled");
    }
  } 
  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    return <Dialog onOpenChange={() => {setEmployee(initialEmployeeState); setIsError(false); setIsSuccess(false); setIsLoading(false); }}>
      <form>
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <ScrollArea className="h-[50vh] w-full rounded-md">
            ...
            {isSuccess && !isError &&
              <Alert>
              <CheckCircle2Icon />
              <AlertTitle>Success! Employee have been created</AlertTitle>
            </Alert>}
          <DialogFooter>
            <DialogClose asChild>
              <Button variant="outline">Cancel</Button>
            </DialogClose>
            <Button ref={btnAddRef} type="submit" onClick={handleAdd}>Add</Button>
          </DialogFooter>
        </DialogContent>
      </form>
    </Dialog>
